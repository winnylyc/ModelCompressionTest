...
def main():
    ...
    model_dir = "saved_models_vgg_normal"
    # model_filename = "resnet18_cifar10.pt"
    quantized_model_filename = "vgg_pre_convert_cifar10.pt"
    model_filepath = 'checkpoint_vgg_normal/ckpt.pth'
    quantized_model_filepath = os.path.join(model_dir,
                                            quantized_model_filename)
    fused_model = VGG('VGG11')
    ...
    fused_model = torch.quantization.fuse_modules(fused_model,
                                                  [["features.0", "features.1", "features.2"], ["features.4", "features.5", "features.6"],
                                                  ["features.8", "features.9", "features.10"], ["features.11", "features.12", "features.13"],
                                                  ["features.15", "features.16", "features.17"], ["features.18", "features.19", "features.20"],
    ...
    root_path2 = 'QuanDiffDeepGra_vgg_normal_20nc/'
    ...
    count_ori_error = 0
    criterion = nn.CrossEntropyLoss()
    myModel.model_fp32.features[0].register_forward_hook(hook)
    myModel.model_fp32.features[4].register_forward_hook(hook)
    myModel.model_fp32.features[8].register_forward_hook(hook)
    myModel.model_fp32.features[11].register_forward_hook(hook)
    myModel.model_fp32.features[15].register_forward_hook(hook)
    myModel.model_fp32.features[18].register_forward_hook(hook)
    myModel.model_fp32.features[22].register_forward_hook(hook)
    myModel.model_fp32.features[25].register_forward_hook(hook)
    # oriModel.features[2].register_forward_hook(hook_ori)
    # oriModel.features[6].register_forward_hook(hook_ori)
    # oriModel.features[10].register_forward_hook(hook_ori)
    # oriModel.features[13].register_forward_hook(hook_ori)
    # oriModel.features[17].register_forward_hook(hook_ori)
    # oriModel.features[20].register_forward_hook(hook_ori)
    # oriModel.features[24].register_forward_hook(hook_ori)
    # oriModel.features[27].register_forward_hook(hook_ori)
    oriModel.features[1].register_forward_hook(hook_ori)
    oriModel.features[5].register_forward_hook(hook_ori)
    oriModel.features[9].register_forward_hook(hook_ori)
    oriModel.features[12].register_forward_hook(hook_ori)
    oriModel.features[16].register_forward_hook(hook_ori)
    oriModel.features[19].register_forward_hook(hook_ori)
    oriModel.features[23].register_forward_hook(hook_ori)
    oriModel.features[26].register_forward_hook(hook_ori)
    for batch_idx, (inputs, targets) in enumerate(testloader):
        # if batch_idx < 500:
        #     continue
        # if batch_idx >= 1000:
        ...
        for p in range(10):
            ...
            while True:
                layer = random.randint(0,7)
                shape_layer = activation[layer].shape
                shape_layer_ori = activation_ori[layer].shape
                # print('compare')
                # print(activation[layer])
                # print(activation_ori[layer])
                activation_index = []
