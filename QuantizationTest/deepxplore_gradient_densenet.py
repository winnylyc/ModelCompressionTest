...
def main():
    ...
    fused_model.load_state_dict(torch.load(model_filepath, map_location=cuda_device)['net'])
    ...
    myModel.load_state_dict(torch.load(quantized_model_filepath, map_location=cuda_device))
    ...
    root_path2 = 'QuanDiffDeepGra_densenets_normal_test/'
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        for p in range(10):
            ...
            x_ori = x.data
            v2 = myModel(x.to(cuda_device))
            v1 = oriModel(x.to(cuda_device))
            r1 = v1.max(1)[1]
            # print(r1)
            if p == 0:
                ori_target = r1
