...
from densenet_relu import DenseNet121
import tqdm
...
def train_model(model,
    ...
    for epoch in range(num_epochs):
        ...
        print("Epoch: {:03d} Train Loss: {:.3f} Train Acc: {:.3f}".format(epoch + 1, train_loss, train_accuracy))
...
def main():
    ...
    model_dir = "saved_models_densenet4"
    model_filename = "densenet_cifar10.pt"
    pre_quantized_model_filename = "densenet_pre_convert_cifar10.pt"
    quantized_model_filename = "densenet_quantized_cifar10.pt"
    model_filepath = 'checkpoint_densenet/ckpt.pth'
    ...
    train_loader, test_loader = prepare_dataloader(num_workers=1,
                                                   train_batch_size=128,
                                                   eval_batch_size=128)
    ...
    #                                               [["dense1.0.conv1", "dense1.0.bn2", "dense1.0.relu2"]],
    #                                               inplace=True)
    # for module_name, module in fused_model.named_children():
    #     if "dense" in module_name:
    #         for Bottleneck_name, Bottleneck in module.named_children():
    #             torch.quantization.fuse_modules(
    #                 Bottleneck, [["bn1", "relu1"],["conv1", "bn2", "relu2"]],
    #                 inplace=True)
    ...
    #         for Bottleneck_name, Bottleneck in module.named_children():
    #             torch.quantization.fuse_modules(
    #                 Bottleneck, [["bn1", "relu1","conv1"],["bn2", "relu2","conv2"]],
    #                 inplace=True)
    ...
    train_model(model=quantized_model,
                ...
                learning_rate=1e-3,
                num_epochs=10)
    save_model(model=quantized_model, model_dir=model_dir, model_filename=pre_quantized_model_filename)
