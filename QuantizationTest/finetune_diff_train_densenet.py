...
def main():
    ...
    trainset = Dataset_finetune_Diff('QuanDiffDeepGra_densenet_normal/')
    # trainloader = torch.utils.data.DataLoader(
    ...
    model_dir = "saved_models_densenet"
    ...
    for epoch in range(50):
        ...
        if acc > best_acc:
            ...
            if not os.path.isdir('checkpoint_densenetQ_finetune_diff_train_normal'):
                os.mkdir('checkpoint_densenetQ_finetune_diff_train_normal')
            torch.save(state, './checkpoint_densenetQ_finetune_diff_train_normal/ckpt.pth')
            best_acc = acc
        print('Test set accuracy:', acc)
    torch.save(state, './checkpoint_densenetQ_finetune_diff_train_normal/ckpt_final.pth')
