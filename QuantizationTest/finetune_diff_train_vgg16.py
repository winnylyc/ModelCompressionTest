...
def main():
    ...
    trainset = Dataset_finetune_Diff('QuanDiffDeepGra_vgg16/')
    # trainloader = torch.utils.data.DataLoader(
    ...
    for epoch in range(50):
        ...
        if acc > best_acc:
            ...
            if not os.path.isdir('checkpoint_vgg16Q_finetune_diff_train_normal'):
                os.mkdir('checkpoint_vgg16Q_finetune_diff_train_normal')
            torch.save(state, './checkpoint_vgg16Q_finetune_diff_train_normal/ckpt.pth')
            best_acc = acc
        print('Test set accuracy:', acc)
    torch.save(state, './checkpoint_vgg16Q_finetune_diff_train_normal/ckpt_final.pth')
